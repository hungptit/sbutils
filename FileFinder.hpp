...
namespace Tools
{
    ...
    class FileFinder
    {
      public:
        ...
        typedef std::unordered_map<std::string, bool> extension_map_type;
        ...
        size_t search(const std::string & folderName, const extension_map_type & extmap, const boost::regex & expressions)
        {
            ...
            for( ; dirIter != endIter ; ++dirIter)
            {
                ...
                if (FileSearchStrategy::apply(fs))
                {
                    ...
                    if (FileExtSearchStrategy::apply(fileExtension, extmap))
                    {
                        ...
                        {
                            Data.emplace_back(std::make_pair(fileName, fs.permissions()));
                        }
                    }
                }
            }
