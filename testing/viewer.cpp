...
namespace {
    class ThreadedLocate {
      public:
        ...
        ThreadedLocate(const std::string &dataFile, const std::vector<std::string> &stems) : Reader(dataFile), Stems(stems) {}
        ThreadedLocate(const std::string &dataFile, const std::vector<std::string> &stems, const std::vector<std::string> &exts)
        ...
        void locate() {
            ...
            for (auto const &aKey : keys) {
                auto const results = find(aKey);
                update(results);
            }
        }

        void locate_t() { // Threaded version of locate
            auto keys = Reader.keys();
            for (auto const &aKey : keys) {
                boost::future<std::vector<Tools::EditedFileInfo>> aThread =
                    boost::async(std::bind(&ThreadedLocate::find, this, aKey));
                auto const results = aThread.get();
                update(results);
            }
        }

        void update(const std::vector<Tools::EditedFileInfo> &results) {
            boost::lock_guard<boost::mutex> guard(UpdateResults);
