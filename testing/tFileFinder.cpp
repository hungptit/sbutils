...
class TestData {
  ...
  private:
    ...
    void init() {
        {
            // Create data folder
            boost::filesystem::path dataFolder = TmpDir / boost::filesystem::path("data");
            ...
            boost::filesystem::permissions(aDataFile, boost::filesystem::owner_read | boost::filesystem::owner_write);
        }

        {        
            // Create src folder
            boost::filesystem::path srcFolder = TmpDir / boost::filesystem::path("src");
            ...
            boost::filesystem::permissions(aSrcFile, boost::filesystem::owner_write | boost::filesystem::owner_read);
        ...
        {
            // Create bin folder
            boost::filesystem::path binFolder = TmpDir / boost::filesystem::path("bin");
            boost::filesystem::create_directories(binFolder);        
            auto aBinFile = binFolder / boost::filesystem::path("test");
            createTestFile(aBinFile);
            boost::filesystem::permissions(aBinFile, boost::filesystem::owner_exe | boost::filesystem::group_exe | 
...
TEST(FileSearchStrategy, Positive) {
    Tools::TemporaryDirectory tmpDir;
    {
        ...
        boost::filesystem::path aFile = tmpDir.getPath() / boost::filesystem::path("src") / boost::filesystem::path("test.cpp");
    ...
    {
        boost::filesystem::path aFile = tmpDir.getPath() / boost::filesystem::path("data") / boost::filesystem::path("data.mat");
        boost::filesystem::file_status fs = boost::filesystem::status(aFile);
        {
            Tools::OwnerReadFiles st;
            EXPECT_TRUE(st.isValid(fs));
...
TEST(FileSearchDefault, Positive) {
    ...
    if (data.size() > 2) {
        EXPECT_TRUE(data[0].first == (tmpDir.getPath() / boost::filesystem::path("bin")).string());
        EXPECT_TRUE(data[1].first == (tmpDir.getPath() / boost::filesystem::path("bin") / boost::filesystem::path("test")).string());
    }
}
